...
class LanguageConfigEntityFormBase extends EntityForm {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    if (\Drupal::moduleHandler()->moduleExists('select_icons')) {
      $classesFlags = $this->getOptionsAttributesFlags();
      $classesLanguages = $this->getOptionsAttributesLanguages();
      $form['source'] = [
        '#type' => 'select_icons',
        '#options_attributes' => $classesLanguages,
      ];

      $form['flag'] = [
        '#type' => 'select_icons',
        '#options_attributes' => $classesFlags,
   ...
   * @return Attribute[]
   */
  protected function getOptionsAttributesFlags() {
    $attributes = array();

    foreach($this->flags as $iso => $country) {
      // Countries are indexed with uppercase codes but CSS classes are lowercase.
      $classes = array('flag', 'flag-' . strtolower($iso));

      // Attribute class implements toString() to the dirty job for us.
      $attributes[$iso] = new Attribute(array('data-class' => $classes));
    }
   ...
   * @return Attribute[]
   */
  protected function getOptionsAttributesLanguages() {
    $attributes = array();
    $languages = \Drupal::service('flags.language_helper')->getAllDefinedLanguages();
